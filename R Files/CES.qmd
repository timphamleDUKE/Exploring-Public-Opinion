...
ces_2024_clean |>
  ...
  group_by(CC24_321b) |>
  summarize(weighted_count = sum(commonweight, na.rm = TRUE)) |>
...
ces_2024_clean |>
  ...
  group_by(poli_party_reg, CC24_321b) |>
  summarize(weighted_count = sum(commonweight, na.rm = TRUE), .groups = "drop") |>
  group_by(poli_party_reg) |>
  mutate(prop = weighted_count / sum(weighted_count)) |>
  ggplot(aes(y = poli_party_reg, x = prop, fill = CC24_321b)) +
  geom_col() +
  scale_x_continuous(labels = scales::comma_format()) +
  labs(
    title = "On the issue of gun regulation, do you support or oppose a 
...
ces_2024_clean |>
  ...
  group_by(poli_party_reg, gender, CC24_321b) |>
  summarize(weighted_count = sum(commonweight, na.rm = TRUE), .groups = "drop") |>
