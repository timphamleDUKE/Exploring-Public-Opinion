...
missing_count <- anes_2024 |>
  summarise(across(everything(), ~ sum(.x < 0 | is.na(.x)))) |>
...
ggplot(least_10_demographic_count, aes(x = neg_count, y = variable)) +
  geom_bar(
    ...
    color = "#b4b5b9") +
  geom_bar(
    data = least_10_demographic_count[1, ],    
    stat = "identity",
    fill = "#7b3ad4",
    color = "#7b3ad4"
  ) +
...
anes_2024_for_missing <- anes_2024 |>
  ...
  replace_with_na_all(condition = ~.x < 0) |>
  mutate(lib_con_7pt = case_when(
    lib_con_7pt == '-9' ~ "Refused",
    lib_con_7pt == '-4' ~ "Error",
    lib_con_7pt == '1' ~ "Extremely liberal",
    ...
    lib_con_7pt == '99' ~ "Haven't thought much about this"
  )) |>
  mutate(lib_con_7pt = factor(lib_con_7pt, levels = c(
    "Extremely liberal",
    "Liberal", 
    "Slightly liberal",
