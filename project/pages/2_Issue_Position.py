...
# Columns with space between Topic and Groups, then closer spacing for Options and Visualization
...
if viz_type == "Agree/Disagree Flow":
    # Use same grouping as traditional Sankey
    ...
    def create_agree_disagree_sankey_holoviews(df, issue_question, list_of_groups, group_type):
        """Create three-layer Sankey using HoloViews to match traditional styling"""
        
        # Filter valid responses
        if group_type == "Ideological Groups":
            group_var = 'lib_con_2pt'
        else:
            group_var = 'poli_party_self_7pt'
        ...
        # Get max value to determine scale type
        max_val = int(df_valid[issue_question].max())
        
        # Create response labels based on scale length - match traditional Sankey labels
        ...
        # Categorize responses into general positions
        def categorize_response(value, max_value):
            if max_value == 7:
                if value <= 3:
                    return "Favor/Support"
                elif value == 4:
                    return "Neither/Neutral"
                else:
                    return "Oppose"
            elif max_value == 5:
                if value <= 2:
        ...
        df_valid['general_position'] = df_valid[issue_question].apply(
            lambda x: categorize_response(x, max_val)
        )
        df_valid['specific_response'] = df_valid[issue_question].map(response_labels)
        
        # Get group labels matching traditional sankey
        if group_type == "Ideological Groups":
        ...
        # Layer 1 -> Layer 2 (Groups -> General Position)
        ...
        # Layer 2 -> Layer 3 (General Position -> Specific Response)
        ...
        # Create HoloViews Sankey
        hv.extension('bokeh')
        
        sankey = hv.Sankey(flows)
        
        # Apply styling with only compatible options
        from holoviews import opts
        
        sankey_styled = sankey.opts(
            opts.Sankey(
                width=900, 
                height=600,
        ...
        return sankey_styled
    
    # Create and display the HoloViews Sankey
    try:
...
# Expander
expander(df_filtered, issue_question)
